...
public class GroupService
{
    ...
    /**
     * @param onlyEnabled
     *            true, wenn nur die aktiven Gruppen geladen werden sollen, sonst false
    ...
    /**
     * Ermittelt, ob es für den aktuell selektierten Mandanten eine Gruppe mit dem als Parameter übergebenen
     * Namen gibt.
     * 
     * @param name
     *            Gruppenname
     * @return Die zum Namen gehörende Gruppe
     */
    public GroupDto getGroupByNameForCurrentClient(String name)
    {
        try
        {
            Group persistentGroup = groupRepo.findGroupByNameForClient(name, authenticationUtil
    ...
    public void saveGroups(List<GroupDto> detachedGroups)
    {
        try
        {
            detachedGroups.forEach(e -> saveGroup(e));
        }
        catch (final Exception e)
        {
            logger.error(e);
